public class Passenger {
    ...
    public Passenger(String line) {
        ...
        try {
            if (Integer.parseInt(data[Constants.PASSENGER_ID]) >= 0 && data[Constants.PASSENGER_ID] != null) {
                this.passengerId = Integer.parseInt(data[Constants.PASSENGER_ID]);
            }
            if (Integer.parseInt(data[Constants.SURVIVED]) == 0 || Integer.parseInt(data[Constants.SURVIVED]) == 1 && data[Constants.SURVIVED] != null) {
                this.survived = Integer.parseInt(data[Constants.SURVIVED]);
            }
            if (Integer.parseInt(data[Constants.P_CLASS]) > 0 && Integer.parseInt(data[Constants.P_CLASS]) <= 3 && data[Constants.P_CLASS] != null) {
            ...
            if (data[Constants.AGE] != "") {
                this.age = Double.parseDouble(data[Constants.AGE]);
            } else this.age = Constants.NULL_AGE;
            if (Integer.parseInt(data[Constants.SIB_SP]) >= 0 && data[Constants.SIB_SP] != null) {
                this.sibSp = Integer.parseInt(data[Constants.SIB_SP]);
            }
            if (Integer.parseInt(data[Constants.PARCH]) >= 0 && data[Constants.PARCH] != null) {
                this.parch = Integer.parseInt(data[Constants.PARCH]);
            }
            this.ticket = data[Constants.TICKET];
            if (Double.parseDouble(data[Constants.FARE]) >= 0 && data[Constants.FARE] != null) {
    ...
    public boolean matchesPClass(String pClass) {
        return Integer.parseInt(pClass) == this.pClass;
    }
